...
def create_robot_with_model(model: GenerativeModel = None):
    offsetX = 250
    offsetY = 100
    return (DifferentialDriveRobot(WIDTH / 2 + offsetX, HEIGHT / 2 + offsetY, random.random() * 360), model if model is not None else GenerativeModel(hidden_size=HIDDEN_LAYER_SIZE))
